...
# Image transformation network is trained using stochastic gradient descent 
# to minimize a weighted combination of loss functions
...
# We use the loss network to define feature reconstruction loss and style reconstruction loss 
# to measure differences in content and style between images
def loss_net(x_in, trux_x_in,width, height,style_image_path,content_weight,style_weight):
    ...
    vgg_output_dict = dict([(layer.name, layer.output) for layer in vgg.layers[-21:]])
...
def add_style_loss(vgg,style_image_path,vgg_layers,vgg_output_dict,img_width, img_height,weight):
    ...
    vgg_style_func = K.function([vgg.layers[-22].input], style_layer_outputs)
